...
- Update `AGENTS.md` "5 places" table + tooling quirk for WS, `CONTRIBUTING.md#architecture-overview` layered diagram + realtime notes + deploy WS limitations (Vercel row), `CONTRIBUTING.md` how-to-add, `README.md` flag.
...
| 9   | Docs + tests + CI gates                                                                      | `AGENTS.md`, `CONTRIBUTING.md#architecture-overview`, `CONTRIBUTING.md`, `README.md`, `skills/**`, `tests/unit/generation-matrix.test.ts` (add messaging dimension or new matrix file), `tests/integration/*`      | 8                   | Full `bun test --timeout 100000`, `bun run check:ci`, `scripts/test-generated.ts --only next-monorepo,tanstack-monorepo` with `--with-messaging`                 |
