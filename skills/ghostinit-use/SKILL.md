...
| `--cache`          | `redis`, `none` (`upstash` alias for redis)               | `none`     | Cache via catalog-pinned Upstash Redis + memory fallback when REPLACE_WITH placeholder; or `none`                                                                                                                                                   |
| `--deploy`         | `vercel`, `fly`, `docker`, `none`                         | `none`     | Docker/Fly use the exact Bun image plus health/grace contracts; Vercel manages patches within `bunVersion: "1.4.x"` while install/build use exact Bun 1.4.0                                                                                         |
...
Cache: `--cache redis` (alias `--cache upstash`) or `--with-cache` enables catalog-pinned Upstash Redis over HTTP (edge/serverless safe, no TCP) + in-memory fallback when `UPSTASH_REDIS_REST_URL` is `REPLACE_WITH_...` placeholder. Auth requires DB (postgres or convex) — validation fails if `--with-auth` with `--database none`.
