  ...
  describe("Next 16.3 TypeScript 7 and Cache Components", () => {
      ...
      for (const database of ["postgres", "convex"] as const) {
        test(`${mode}/${database} removes legacy segment config and keeps GET boundaries explicit`, () => {
          ...
          for (const path of [
            ...
            expect(requestScoped).toMatch(
              /request\.(?:headers|method|url)|new URL\(request\.url\)/,
            );
    ...
    for (const mode of ["monorepo", "single"] as const) {
      test(`${mode} puts session and request-locale work behind intentional boundaries`, () => {
        ...
        expect(resetPassword).toContain("async function ResetPasswordContent");
        expect(resetPassword).toContain("const query = await searchParams;");
        expect(resetPassword).toContain(
          "<Suspense fallback={<ResetPasswordLoading />}><ResetPasswordContent {...props} /></Suspense>",
        );
        expect(resetPassword).not.toContain('"use cache"');
        const resetPasswordScreen = read(
          files,
          `${sourceRoot}/features/auth/reset-password-screen.tsx`,
        );
        expect(resetPasswordScreen).toContain('"use client"');
        expect(resetPasswordScreen).toContain("useResetPasswordForm(token, queryError)");
        expect(read(files, `${appRoot}/forgot-password/page.tsx`)).toContain(
          "<RequestLocalizedMetadataBoundary />",
        );
        expect(read(files, `${sourceRoot}/lib/request-localized-metadata.tsx`)).toContain(
          "<Suspense fallback={null}><RequestLocalizedMetadata /></Suspense>",
        );
        expect(read(files, `${appRoot}/maintenance/page.tsx`)).toContain("<Suspense");
        expect(read(files, `${appRoot}/not-found.tsx`)).toContain(
          'export { NotFoundScreen as default } from "@/features/system/not-found"',
        );
        expect(read(files, `${sourceRoot}/features/system/not-found.tsx`)).toContain("<Suspense");
        if (mode === "monorepo") {
          const marketing = read(files, `${appRoot}/page.tsx`);
          expect(marketing.match(/<Suspense/g)).toHaveLength(4);
          for (const specialPage of ["unauthorized", "forbidden"]) {
            expect(read(files, `${appRoot}/${specialPage}.tsx`)).toContain(
              `from "@/features/system/${specialPage}"`,
            );
            expect(read(files, `${sourceRoot}/features/system/${specialPage}.tsx`)).toContain(
              "<Suspense",
            );
          }
        }
