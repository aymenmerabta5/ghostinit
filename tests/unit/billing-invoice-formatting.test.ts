...
describe.each(["stripe", "paddle", "polar"] as const)("%s billing selection", (globalProvider) => {
  ...
  function createElement(
    ...
    if (typeof type === "function") return type({ ...props, children }) as Element;
  ...
  function load(
    ...
    let source = output.read(path);
    const sourceRoot = path.startsWith("apps/mobile/")
      ? "apps/mobile/src"
      : path.startsWith("apps/desktop/")
        ? "apps/desktop/src/renderer"
        : output.root;
    if (component === "BillingView")
      source +=
        "\n" +
        ["subscriptions-card", "invoices-card"]
          .map((name) => output.read(`${sourceRoot}/features/billing/components/${name}.tsx`))
          .join("\n");
    ...
    const bindings: Record<string, unknown> = {
      ...statusFunctions(output.read(sourceRoot + "/features/billing/status-labels.ts")),
      React: {
        createElement,
    ...
    const screenSource =
      component === "BillingView" ? output.read(`${sourceRoot}/features/billing/screen.tsx`) : "";
    const runtimeImports = screenSource
      ? parseSync("screen.tsx", screenSource).program.body.filter(
          (entry) => entry.type === "ImportDeclaration",
        )
      : imports;
    ...
    if (screenSource) {
      const screenBindings = {
        ...
        selectedProviders: [],
        useBillingSnapshot: () => bindings.nativeProps,
        useBillingActions: () => ({ error: null, isPending: false, run() {}, submit() {} }),
      };
      let screenRuntime = screenSource;
      for (const entry of [...runtimeImports].reverse())
        screenRuntime = screenRuntime.slice(0, entry.start) + screenRuntime.slice(entry.end);
      const renderScreen = new Function(
        ...Object.keys(screenBindings),
        transpiler.transformSync(screenRuntime.replace(/^export /gm, "")) +
          "\nreturn BillingScreen;",
      )(...Object.values(screenBindings)) as () => Element;
      const rendered = renderScreen();
      expect(rendered.type).toBe("BillingView");
      expect(rendered.props.locale).toBe(locale);
      bindings.nativeProps = rendered.props;
    }
    return new Function(
      ...Object.keys(bindings),
      transpiler.transformSync(stripped) +
        `\nreturn { render: ${component === "BillingView" ? "(props) => BillingView({ ...nativeProps, ...props })" : component}, formatBillingInvoiceAmount };`,
  ...
  describe("provider invoice monetary presentation", () => {
      ...
      for (const framework of ["nextjs", "tanstack-start"] as const)
        for (const database of ["postgres", "convex"] as const) {
          test(`${mode}/${framework}/${database} renders actual invoice JSX with currency-aware provider units`, () => {
            ...
            const path =
              framework === "nextjs"
                ? `${output.root}/features/billing/components/billing-invoices.tsx`
                : `${output.root}/features/billing/components/billing-invoices.tsx`;
        ...
        for (const database of ["postgres", "convex"] as const) {
          test(`${mode}/${database} Stripe-specific JSX uses the same zero/two/three-decimal contract`, () => {
            ...
            const component = load(
              output,
              `${output.root}/features/billing/components/providers/stripe-invoices.tsx`,
              "StripeInvoices",
              selected,
            );
    ...
    test("formatted JSX follows the selected locale without changing persisted values", () => {
      ...
      for (const locale of ["fr-FR", "ar-DZ"]) {
        const component = load(
          output,
          "src/features/billing/components/billing-invoices.tsx",
          "BillingInvoices",
          records,
          locale,
        );
