...
describe.each(["stripe", "paddle", "polar"] as const)("%s billing selection", (globalProvider) => {
  ...
  function dynamicImportSpecifiers(source: string): string[] {
    return [...source.matchAll(/\bimport\(\s*["']([^"']+)["']\s*\)/g)].map(
      (match) => match[1] ?? "",
    );
  }
    ...
    for (const mode of ["monorepo", "single"] as const) {
      for (const database of ["postgres", "convex"] as const) {
        test(`${mode}/${database} keeps every server dependency outside the physical route tree`, () => {
          ...
          // Two selected webhook routes replace the old four-provider fixture.
          expect(apiRoutes).toHaveLength(database === "postgres" ? 12 : 11);
          ...
          for (const provider of ["chargily", globalProvider] as const) {
