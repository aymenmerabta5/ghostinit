...
describe.each(["stripe", "paddle", "polar"] as const)("%s billing selection", (globalProvider) => {
  ...
  const CASES: Array<{ name: string; config: ProjectConfig }> = [
    ...
    {
      name: "Convex web and native capabilities stay behind typed client boundaries",
      config: config({
        mode: "monorepo",
        preset: "saas",
        framework: "tanstack-start",
        database: "convex",
        billing: ["chargily", globalProvider, "manual"],
        apps: ["web", "mobile", "desktop"],
        messaging: true,
        storage: true,
        notifications: true,
  ...
  describe("generated architecture checker", () => {
    ...
    test("single PDF keeps its web client outside the server capability tree", async () => {
      ...
      const taint = report.findings.filter(
        ({ id, file }) =>
          id === "client-transitive-server-import" && file === "src/hooks/usePdf.ts",
      );
      expect(report.complete, JSON.stringify(report.findings, null, 2)).toBe(true);
