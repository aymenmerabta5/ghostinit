...
const CORNERS: ReadonlyArray<readonly [string, Partial<CreateResolutionInput>]> = [
  ...
  ["tanstack", { framework: "tanstack-start", billing: ["stripe", "paddle", "polar"] }],
  ...
  ["feature-flags", { preset: "custom", database: "none", withApi: true, featureFlags: "posthog" }],
    ...
    for (const [id, overrides] of CORNERS) {
      ...
      for (const file of files) {
        ...
        const hasPlaywrightSpecs = files.some(
          (candidate) =>
            candidate.path.startsWith(`${packageRoot}e2e/`) && candidate.path.endsWith(".spec.ts"),
        );
