...
describe.each(["stripe", "paddle", "polar"] as const)("%s billing selection", (globalProvider) => {
  ...
  describe("maximal Cloudflare native profile", () => {
    test("keeps Convex mobile auth ownership and native clients type-directed", () => {
      ...
      const mobileAdapter = read(files, "apps/mobile/src/features/messaging/queries.ts");
      ...
      for (const path of [
        "apps/mobile/src/features/admin-users/queries.ts",
        "apps/mobile/src/features/billing/queries.ts",
        "apps/mobile/src/features/settings/queries.ts",
      ]) {
        expect(read(files, path), path).toContain("orpc.me.queryOptions");
      }
      const dashboard = read(files, "apps/mobile/src/features/dashboard/model.ts");
