...
const EMAIL_DEPENDENCIES = ["resend", "react-email"] as const;
const DEPRECATED_REACT_EMAIL_DEPENDENCIES = [
  "@react-email/components",
  "@react-email/render",
  "@react-email/tailwind",
] as const;
const EMAIL_DEPENDENCY_SURFACE = [
  ...EMAIL_DEPENDENCIES,
  ...DEPRECATED_REACT_EMAIL_DEPENDENCIES,
] as const;
...
describe("generated email capability", () => {
  for (const corner of EMAIL_MATRIX) {
    test(`${corner.key} follows the complete email boundary`, () => {
      ...
      const emailLayoutPath =
        corner.mode === "monorepo"
          ? "packages/email/src/templates/EmailLayout.tsx"
          : "src/server/email/templates/EmailLayout.tsx";
      const emailRenderTestPath = "packages/email/tests/render.test.ts";
      const authPath =
      ...
      if (corner.email) {
        ...
        for (const dependency of DEPRECATED_REACT_EMAIL_DEPENDENCIES) {
          expect(
            manifest.dependencies?.[dependency],
            `${corner.key}: ${dependency}`,
          ).toBeUndefined();
        }
        if (corner.mode === "monorepo") {
          const emailTsconfig = JSON.parse(read("packages/email/tsconfig.json")) as {
            compilerOptions?: { types?: string[] };
            include?: string[];
          };
          expect(manifest.devDependencies?.["bun-types"]).toBeDefined();
          expect(emailTsconfig.compilerOptions?.types).toContain("bun-types");
          expect(emailTsconfig.include).toContain("tests/**/*");
          const renderTest = read(emailRenderTestPath);
          expect(renderTest).toContain("satisfies TailwindConfig");
          expect(renderTest).toContain("const html = await render(");
          expect(renderTest).toContain('expect(html).toContain("<!DOCTYPE html")');
        }
        ...
        const layout = read(emailLayoutPath);
        const emailSource = corner.files
          .filter(({ path }) =>
            corner.mode === "monorepo"
              ? path.startsWith("packages/email/")
              : path.startsWith("src/server/email/"),
          )
          .map(({ content }) => content)
          .join("\n");
        expect(send).toContain('import { render } from "react-email"');
        expect(layout).toContain('from "react-email"');
        expect(layout).toContain("type TailwindConfig");
        for (const dependency of DEPRECATED_REACT_EMAIL_DEPENDENCIES) {
          expect(emailSource, `${corner.key}: source import ${dependency}`).not.toContain(
            dependency,
          );
        }
        ...
        for (const dependency of EMAIL_DEPENDENCY_SURFACE) {
          for (const path of [
            "package.json",
        ...
        expect(read(emailRenderTestPath)).toBe("");
