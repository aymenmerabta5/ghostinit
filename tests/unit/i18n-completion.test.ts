  ...
  describe("completed application localization", () => {
    ...
    for (const mode of ["monorepo", "single"] as const) {
      for (const framework of ["nextjs", "tanstack-start"] as const) {
        test(`${mode}/${framework} translates recovery, settings, common states, and root errors`, () => {
          ...
          const magicLink = read(files, `${root}/features/auth/components/magic-link-form.tsx`);
          const verifyEmail = read(files, `${root}/features/auth/components/verify-email-form.tsx`);
          for (const flow of [magicLink, verifyEmail]) {
            ...
            expect(flow).not.toMatch(
              />\s*(?:Secure email flow|Request failed|Email requested)\s*</,
            );
          ...
          const settings = [
            ...
            ...files
              .filter(({ path }) => path.startsWith(`${root}/features/settings/`))
              .map(({ content }) => content),
          ].join("\n");
          ...
          if (framework === "nextjs") {
            ...
            expect(read(files, `${root}/features/system/route-fallbacks.tsx`)).toContain(
              'useStandaloneSurfaceTranslations("errors")',
            );
            expect(read(files, `${root}/features/system/route-fallbacks.tsx`)).toContain(
              't("notFound.shortDescription")',
            );
    ...
    test("translates provider billing details without translating protocol identifiers", () => {
      ...
      const billing = files
        .filter(
          ({ path }) =>
            (path.startsWith(`${root}/app/billing/`) ||
              path.startsWith(`${root}/features/billing/`)) &&
            path.endsWith(".tsx"),
        )
        .map(({ content }) => content)
        .join("\n");
      ...
      for (const key of [
        ...
        ...{
          stripe: ["stripeFlowDescription"],
          paddle: ["paddleTaxDescription"],
          polar: ["polarFlowDescription", "licenseDescription", "usageDescription", "seats"],
        }[globalProvider],
      ]) {
    ...
    test("executes the emitted PDF locale runtime and wires all rendered templates", async () => {
      ...
      for (const mode of ["monorepo", "single"] as const) {
        ...
        const web = read(files, `${sourceRoot(mode)}/features/pdf/use-pdf-workspace.ts`);
        expect(web).toContain("samplePdfData(template, t)");
        expect(read(files, `${sourceRoot(mode)}/features/pdf/pdf-workspace.tsx`)).toContain(
          "usePdfWorkspace",
        );
