...
import { BILLING_PROVIDERS } from "../../src/domain/project/choices.js";
...
describe("provider-specific billing operation promises", () => {
  for (const mode of ["monorepo", "single"] as const) {
    test(`${mode} generated provider metadata typechecks every advertised operation`, async () => {
      ...
      try {
        ...
        // Isolate the external DTO dependency; compile the actual emitted declaration and values.
        await transaction.write(
          "queries.ts",
          `export type ProviderName = ${BILLING_PROVIDERS.map((name) => JSON.stringify(name)).join(" | ")};\n`,
        );
        await transaction.write(
          "tsconfig.json",
          JSON.stringify({
  ...
  test("domain support and emitted UI controls agree for each provider", () => {
    ...
    for (const option of billingClientProviderOptions(BILLING_PROVIDERS)) {
  ...
  for (const framework of ["nextjs", "tanstack-start"] as const) {
    for (const provider of ["stripe", "chargily"] as const) {
      test(`${framework}/${provider} requires its real operations across all app targets`, () => {
        ...
        const unavailable = provider === "stripe" ? "billing.payment-link.v1" : "billing.portal.v1";
        ...
        const incomplete = {
          ...
          files: plan.files.map((file) => ({
            ...
            provenance: {
              ...file.provenance,
              acceptance: file.provenance.acceptance.filter((operation) => operation !== required),
            },
          })),
        };
        try {
