...
describe("billing provider — chargily (Algeria EDAHABIA/CIB checkout-only server-only)", () => {
  ...
  it("chargily webhook Next.js route file contains arrayBuffer Buffer.from verifySignature 400 403 200 ok", () => {
    const addonMap = {
      stripe: { inUse: false },
      chargily: { inUse: true },
      paddle: { inUse: false },
    ...
    expect(route).toContain("Buffer.from(await req.arrayBuffer())");
  ...
  it("chargily provider does NOT support portal — throws NOT_SUPPORTED", () => {
    const addonMap = {
      stripe: { inUse: false },
      chargily: { inUse: true },
      paddle: { inUse: false },
    ...
    const files = billingFiles({ mode: "monorepo", addons: addonMap as never });
    const content = aggProvider(files, "chargily");
    expect(content).toContain("NOT_SUPPORTED");
  ...
  it("env handling: placeholders REPLACE_WITH... in root.ts + secret() real generation + t3env server-only + turbo globalEnv", async () => {
    ...
    const configEnv =
      pkgFiles.find((f: any) => f.path.includes("config/src/env.ts"))?.content ?? "";
    expect(configEnv).toContain("CHARGILY_API_KEY");
    expect(configEnv).toContain("CHARGILY_SECRET_KEY");
    expect(configEnv).toContain("CHARGILY_MODE");
    expect(configEnv.includes("NEXT_PUBLIC_CHARGILY")).toBe(false);
    expect(configEnv).toContain("NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY");
    expect(configEnv).toContain("NEXT_PUBLIC_PADDLE_CLIENT_TOKEN");
  });
  ...
  it("eve templates include billing-renewal.md monthly cron for chargily manual recurring via DB + checkout", () => {
    const files = eveFiles("testapp", "bun");
    const renewalMd = files.find((f: any) => f.path.includes("billing-renewal.md"))?.content ?? "";
    const renewalExample =
      files.find((f: any) => f.path.includes("billing-renewal.example.ts"))?.content ?? "";
