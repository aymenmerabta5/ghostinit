...
describe("ghostinit init", () => {
  ...
  it("honors Cloudflare deployment options through the init pipeline", () => {
    const result = run(
      [
        "init",
        "worker-app",
    ...
    expect(result.status).toBe(0);
    expect(existsSync(join(tmp, "apps", "web", "wrangler.jsonc"))).toBe(true);
    const state = JSON.parse(readFileSync(join(tmp, ".ghostinit", "state.json"), "utf-8")) as {
      project: { deploy: string };
    };
    expect(state.project.deploy).toBe("cloudflare");
  });
});
