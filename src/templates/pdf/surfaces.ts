...
export function PdfWorkspace(): React.JSX.Element {
  ...
  async function download(): Promise<void> {
    await generate({ template, data: samplePdfData(template, t), locale, fileName: \`\${template}.pdf\` });
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-6 p-6">
      ...
      <Card><CardHeader><CardTitle>{t("template")}</CardTitle></CardHeader><CardContent className="flex flex-col gap-4">
        ...
        <Button type="button" disabled={loading} onClick={() => void download()}>{loading ? t("generating") : t("generateDownload")}</Button>
      </CardContent></Card>
    </main>
  );
}`;
