...
export function PdfWorkspace(): React.JSX.Element {
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-7 px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
      <div className="flex flex-col gap-2"><h1 className="text-3xl font-semibold tracking-tight">{t("secureTitle")}</h1><p className="max-w-[65ch] text-sm leading-6 text-muted-foreground">{t("webDescription")}</p></div>
      <Card className="max-w-3xl"><CardHeader><CardTitle as="h2">{t("documentWorkspace")}</CardTitle></CardHeader><CardContent className="flex flex-col gap-5">
...
${labels}
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-6"><div className="flex items-start justify-between gap-4"><div className="flex flex-col gap-2"><h1 className="text-3xl font-semibold tracking-tight">${i18n.child("secureTitle", "Create a PDF")}</h1><p className="max-w-[65ch] text-sm text-muted-foreground">${i18n.child("desktopDescription", "Uses the same authenticated, bounded renderer as the web and mobile apps.")}</p></div><Button variant="outline" render={<Link to="/dashboard" />} nativeButton={false}>${i18n.child("dashboard", "Dashboard")}</Button></div><Card><CardHeader><CardTitle as="h2">${i18n.child("documentWorkspace", "Document workspace")}</CardTitle></CardHeader><CardContent className="flex flex-col gap-4"><Field><FieldLabel id="pdf-template-label">${i18n.child("template", "Template")}</FieldLabel><Select items={templateOptions} value={template} onValueChange={(value) => { if (value === "invoice" || value === "certificate" || value === "agreement") setTemplate(value); }}><SelectTrigger aria-labelledby="pdf-template-label"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{templateOptions.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>{error ? <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert> : null}<Button type="button" disabled={loading} aria-busy={loading} onClick={() => void download()}>{loading ? ${i18n.value("generating", "Generating…")} : ${i18n.value("generateDownload", "Generate and download")}}</Button></CardContent></Card></main>
  );
}
`;
}
