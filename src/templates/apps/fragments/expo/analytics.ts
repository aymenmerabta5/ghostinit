...
export function expoAnalyticsContent(_mode: "monorepo" | "single" = "monorepo"): string {
  return `import * as React from "react";
import { useSegments } from "expo-router";
import { PostHogProvider, usePostHog } from "posthog-react-native";
import { getClient } from "./analytics-client";
import { EVENT_NAME, validProperties, type ExpoAnalyticsAdapter, type ExpoAnalyticsProviderProps } from "./analytics-model";
export { resolveExpoAnalyticsConfig } from "./analytics-client";
export type { AnalyticsPrimitive, AnalyticsProperties, ExpoAnalyticsAdapter, ExpoAnalyticsProviderProps } from "./analytics-model";
...
export function expoAnalyticsClientContent(mode: "monorepo" | "single" = "monorepo"): string {
...
export function getClient(initiallyOptedOut: boolean): PostHog | null {
...
export function expoAnalyticsModelContent(): string {
...
export const EVENT_NAME = /^[a-zA-Z0-9][a-zA-Z0-9 ._/-]{0,127}$/;
const PROPERTY_NAME = /^[a-zA-Z0-9_$][a-zA-Z0-9_$.-]{0,63}$/;
export function validProperties(value: AnalyticsProperties | undefined): boolean {
...
export function expoAnalyticsFiles(mode: "monorepo" | "single" = "monorepo"): TemplateFile[] {
  const root = mode === "monorepo" ? "apps/mobile/src/lib" : "src/lib";
  return [
    expoAnalyticsFile(mode),
    file(`${root}/analytics-client.ts`, expoAnalyticsClientContent(mode)),
    file(`${root}/analytics-model.ts`, expoAnalyticsModelContent()),
  ];
}
