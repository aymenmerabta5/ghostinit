        ...
        <Card className="flex flex-col gap-4 p-4">
          ...
          <CardContent className="flex flex-col gap-4 p-0">
            ...
            <form onSubmit={(e) => { e.preventDefault(); if (!input.trim() || isBusy) return; agent.send({ message: input }); setInput(""); }} className="flex gap-2">
              <Input value={input} onChange={(e) => setInput(e.target.value)} placeholder={isBusy ? "Agent working..." : "Ask to scaffold a module, check architecture, sync registries..."} disabled={isBusy} className="flex-1 bg-background" />
