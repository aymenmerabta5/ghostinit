...
${i18n.hookLine}
  return <main className="mx-auto flex max-w-5xl flex-col gap-6"><h1 className="text-2xl font-semibold tracking-tight">${i18n.child("title", "Billing")}</h1><ManualBillingPanel /></main>;
}
`;
  }
...
import { formatBillingSubscriptionStatus, formatBillingInvoiceStatus } from "${mode === "single" ? "@/renderer/features/billing/status-labels" : "@/features/billing/status-labels"}";
