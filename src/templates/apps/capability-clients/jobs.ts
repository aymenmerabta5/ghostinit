...
${i18n.hookLine}
  ...
  return <ScrollView className="flex-1 bg-background"><View className="gap-4 p-5">
    ...
    <Card><CardHeader><CardTitle>${i18n.child("runTitle", "Start a sample job")}</CardTitle><CardDescription>${i18n.child("echoPayload", "Message")}</CardDescription></CardHeader><CardContent className="gap-3"><Input editable={!isPending} value={message} onChangeText={setMessage} placeholder={${i18n.value("echoPayload", "Message")}} /><Button disabled={isPending} accessibilityState={{ busy: isPending }} onPress={enqueue}>${i18n.child("enqueue", "Run sample job")}</Button></CardContent></Card>
    <Card><CardHeader><CardTitle>${i18n.child("statusTitle", "Job status")}</CardTitle><CardDescription>${i18n.child("refresh", "Refresh or cancel an owned run")}</CardDescription></CardHeader><CardContent className="gap-3"><Input editable={!isPending} value={runId} onChangeText={setRunId} placeholder={${i18n.value("runId", "Run ID")}} /><View className="flex-row gap-2"><Button disabled={isPending} accessibilityState={{ busy: isPending }} className="flex-1" variant="outline" onPress={refresh}>${i18n.child("refresh", "Refresh")}</Button><Button disabled={isPending} accessibilityState={{ busy: isPending }} className="flex-1" variant="destructive" onPress={cancel}>${i18n.child("cancel", "Cancel")}</Button></View></CardContent></Card>
