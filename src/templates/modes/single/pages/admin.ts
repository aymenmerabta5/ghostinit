...
export function useAdminUsersHookSingleContent(): string {
  return [
    ...
    "import type { AdminUser as KernelAdminUser } from '@/lib/kernel';",
    "export type AdminUser = KernelAdminUser;",
    "export function useAdminUsers() {",
    "  const [data, setData] = useState<{ users: AdminUser[]; total: number } | null>(null);",
    "  const [error, setError] = useState<string | null>(null);",
    "  const [loading, setLoading] = useState(true);",
    "  const refresh = useCallback(async () => { setError(null); setLoading(true); try { const result = await authClient.admin.listUsers({ query: { limit: 100 } }); if (result.error) { setError(result.error.message ?? 'Failed'); return; } if (result.data) setData({ users: result.data.users.map((user) => ({ id: user.id, name: user.name, email: user.email, role: user.role ?? 'user', banned: user.banned ?? false })), total: result.data.total }); } finally { setLoading(false); } }, []);",
    ...
    "  const setRole = useCallback(async (userId: string, currentRole: string) => { const role = currentRole === 'admin' ? 'user' : 'admin'; await authClient.admin.setRole({ userId, role }); await refresh(); }, [refresh]);",
...
export function adminCreateUserPageSingleFileContent(): string {
  return [
    ...
    "  return (<div className='mx-auto flex w-full max-w-xl flex-col gap-8 p-6 md:p-0'><div className='flex flex-col gap-2'><div className='flex items-center justify-between gap-4'><h1 className='text-2xl font-semibold tracking-tight'>Create user</h1><Button variant='ghost' size='sm' asChild><Link href='/admin/users'>Back</Link></Button></div><p className='text-sm text-muted-foreground'>Add new account.</p></div><Separator /><Card><CardHeader><CardTitle>User details</CardTitle><CardDescription>Password &ge;8 Role determines access.</CardDescription></CardHeader><CardContent className='flex flex-col gap-6'>{error ? <Alert variant='destructive'><AlertTitle>Failed</AlertTitle><AlertDescription>{error}</AlertDescription></Alert> : null}<Form form={form} className='flex flex-col gap-6'><FieldGroup><TanStackField form={form} name='name'>{(f) => (<Field><FieldLabel>Name</FieldLabel><Input value={f.state.value} onChange={(e) => f.handleChange(e.target.value)} onBlur={f.handleBlur} /></Field>)}</TanStackField><TanStackField form={form} name='email' validators={{ onSubmit: ({ value }) => (value.includes('@') ? undefined : 'Enter valid email') }}>{(f) => (<Field><FieldLabel>Email</FieldLabel><Input type='email' value={f.state.value} onChange={(e) => f.handleChange(e.target.value)} onBlur={f.handleBlur} /></Field>)}</TanStackField><TanStackField form={form} name='password' validators={{ onSubmit: ({ value }) => (value.length >= 8 ? undefined : 'Password >=8') }}>{(f) => (<Field><FieldLabel>Password</FieldLabel><Input type='password' value={f.state.value} onChange={(e) => f.handleChange(e.target.value)} onBlur={f.handleBlur} /></Field>)}</TanStackField><TanStackField form={form} name='role'>{(f) => (<Field><FieldLabel>Role</FieldLabel><select value={f.state.value} onChange={(e) => f.handleChange(e.target.value === 'admin' ? 'admin' : 'user')} onBlur={f.handleBlur} className='flex h-10 w-full rounded-md border bg-background px-3 text-sm'><option value='user'>User</option><option value='admin'>Admin</option></select></Field>)}</TanStackField></FieldGroup><SubmitButton>Create user</SubmitButton></Form></CardContent></Card></div>);",
    "}",
    "",
  ].join("\n");
}
