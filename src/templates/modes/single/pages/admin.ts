...
export function useAdminUsersHookSingleContent(): string {
  return [
    ...
    "  const setRole = useCallback(async (userId: string, currentRole: string) => { const role = currentRole === 'admin' ? 'user' : 'admin'; await authClient.admin.setRole({ userId, role: role as unknown as string }); await refresh(); }, [refresh]);",
