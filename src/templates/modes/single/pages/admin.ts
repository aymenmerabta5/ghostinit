...
export function useAdminUsersHookSingleContent(): string {
  return [
    ...
    "  const setRole = useCallback(async (userId: string, currentRole: string) => { const role: 'admin' | 'user' = currentRole === 'admin' ? 'user' : 'admin'; await authClient.admin.setRole({ userId, role }); await refresh(); }, [refresh]);",
