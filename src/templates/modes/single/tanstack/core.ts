...
export function singleViteConfigTanstackContent(hasCloudflare = false): string {
  return [
...
export function singleRootRouteTanstackContent(): string {
  return [
    ...
    "          <p className='text-sm text-muted-foreground max-w-[65ch] mt-2'>{error instanceof Error ? error.message : String(error)}</p>",
    "        </div>",
    "      </main>",
    "    </RootDocument>",
    "  ),",
