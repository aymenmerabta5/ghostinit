export function singleSignInRouteTanstackContent(): string {
  return [
    ...
    "      if ((result.data as any)?.twoFactorRedirect) { void navigate({ to: '/2fa' }); return }",
...
export function singleResetPasswordRouteTanstackContent(): string {
  return [
    ...
    "  const token = (search as any)?.token ?? (typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('token') : '')",
    "  useEffect(() => { if (!token) setError('Missing reset token. Use the link from your email.') }, [token])",
    "  const form = useForm({",
    ...
    "      const result = await authClient.resetPassword({ newPassword: value.newPassword, token } as any)",
    ...
    "            {!token ? (<div className='flex flex-col gap-4'><p className='text-sm text-muted-foreground'>No valid token found.</p><Link to='/forgot-password'><Button>Request new link</Button></Link></div>) : (",
    "              <Form form={form} className='flex flex-col gap-6'>",
    "                <FieldGroup>",
