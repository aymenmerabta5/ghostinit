...
export function singleResetPasswordRouteTanstackContent(): string {
  return [
    ...
    "  const token = (search as unknown as string)?.token ?? (typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('token') : '')",
    ...
    "      const result = await authClient.resetPassword({ newPassword: value.newPassword, token } as unknown as string)",
