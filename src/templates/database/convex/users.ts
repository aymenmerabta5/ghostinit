...
export function convexUsersContent(): string {
  return [
    ...
    "    const user = await authComponent.getAuthUser(ctx);",
    '    if (!user || (user as unknown as { role?: string }).role !== "admin") throw new ConvexError({ code: "FORBIDDEN", message: "Admin only" });',
    ...
    '    if (current._id !== args.id && (current as unknown as { role?: string }).role !== "admin") throw new ConvexError({ code: "FORBIDDEN", message: "Forbidden" });',
