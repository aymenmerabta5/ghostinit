...
function baseChargilyNext(imp: DbImports): string {
  return [
    ...
    '  try { if (!db.query.webhook_events.findFirst) { throw new Error("webhook_events query not available — ensure billing schema is migrated"); } const existing = await db.query.webhook_events.findFirst({ where: eq(webhook_events.providerEventId, eventId), }); if (existing?.processed) { logger.info(`[chargily webhook] already processed ${eventId}`); return new Response("already processed", { status: 200 }); } } catch (error) { const message = error instanceof Error ? error.message : String(error); logger.error(`[chargily webhook] idempotency check failed: ${message}`); return new Response(`Idempotency store unavailable: ${message}`, { status: 500, headers: { "Retry-After": "60" } }); }',
    '  try { const status = (data.status as string | undefined) ?? (payload.status as string | undefined); const checkoutId = (data.id as string | undefined) ?? (payload.id as string | undefined); switch (eventType) { case "checkout.paid": case "checkout.completed": case "checkout_paid": { if (checkoutId) { try { await db.update(checkouts).set({ status: "completed" }).where(eq(checkouts.providerCheckoutId, checkoutId)); } catch (error) { logger.error(`[chargily webhook] update checkout completed failed: ${error instanceof Error ? error.message : String(error)}`); } try { const meta = ((data as unknown as Record<string, unknown>).metadata as Record<string, unknown> | undefined) ?? (payload.metadata as Record<string, unknown> | undefined); const userId = (meta?.userId as string | undefined) ?? (meta?.chargily_user_id as string | undefined); if (userId) { await db.insert(subscriptions).values({ userId, provider: "chargily", providerSubscriptionId: checkoutId, status: "active", metadata: meta as unknown as Record<string, unknown>, }).onConflictDoNothing(); } } catch (error) { logger.error(`[chargily webhook] create subscription failed: ${error instanceof Error ? error.message : String(error)}`); } } break; } case "checkout.failed": case "checkout.canceled": case "checkout.expired": { if (checkoutId) { try { const mapped = eventType.includes("failed") ? "failed" : eventType.includes("canceled") ? "failed" : "expired"; await db.update(checkouts).set({ status: mapped as unknown as string }).where(eq(checkouts.providerCheckoutId, checkoutId)); } catch (error) { logger.error(`[chargily webhook] update checkout ${eventType} failed: ${error instanceof Error ? error.message : String(error)}`); } if (eventType.includes("failed") || eventType.includes("canceled")) { try { await db.update(subscriptions).set({ status: "canceled" as unknown as string }).where(eq(subscriptions.providerSubscriptionId, checkoutId)); } catch (error) { logger.error(`[chargily webhook] cancel subscription failed: ${error instanceof Error ? error.message : String(error)}`); } } } break; } default: { if (checkoutId && status) { try { const mapStatus = (s: string): string => { if (s === "paid" || s === "completed") return "completed"; if (s === "failed" || s === "canceled" || s === "cancelled") return "failed"; if (s === "expired") return "expired"; if (s === "pending" || s === "processing" || s === "open") return "pending"; return "pending"; }; await db.update(checkouts).set({ status: mapStatus(status) as unknown as string }).where(eq(checkouts.providerCheckoutId, checkoutId)); } catch (error) { logger.error(`[chargily webhook] generic status update failed: ${error instanceof Error ? error.message : String(error)}`); } } break; } } } catch (error) { logger.error(`[chargily webhook] handler error: ${error instanceof Error ? error.message : String(error)}`); }',
    '  try { await db.insert(webhook_events).values({ provider: "chargily", providerEventId: eventId, type: eventType, payload: payload as unknown as Record<string, unknown>, processed: true, }).onConflictDoNothing(); } catch (error) { logger.error(`[chargily webhook] failed to record event: ${error instanceof Error ? error.message : String(error)}`); }',
...
function baseChargilyConvexNext(imp: DbImports): string {
  ...
  return [
    ...
    "  const data = (payload.data as unknown as Record<string, unknown>) ?? (payload.checkout as unknown as Record<string, unknown>) ?? payload;",
    ...
    '  try { const existing = await convexClient.query(api.billing.checkWebhookEvent, { provider: "chargily", providerEventId: eventId }); if ((existing as unknown as { processed?: boolean })?.processed) { logger.info(`[chargily] already processed ${eventId}`); return new Response("already processed", { status: 200 }); } } catch (e) { const _cmsg = e instanceof Error ? e.message : String(e); logger.error(`[chargily] idempotency check failed ${_cmsg}`); return new Response(`Idempotency store unavailable: ${_cmsg}`, { status: 500, headers: { "Retry-After": "60" } }); }',
    '  try { const status = data.status ?? payload.status; const checkoutId = data.id ?? payload.id; if (eventType === "checkout.paid" || eventType === "checkout.completed" || status === "paid") { if (checkoutId) { await convexClient.mutation(api.billing.upsertCheckout, { provider: "chargily", providerCheckoutId: checkoutId as string, status: "completed" as unknown as string, metadata: (data.metadata ?? payload.metadata) as unknown as Record<string, unknown> }); const meta = data.metadata ?? payload.metadata; const userId = (meta as unknown as Record<string, unknown>)?.userId; if (userId) { await convexClient.mutation(api.billing.upsertSubscription, { userId, provider: "chargily", providerSubscriptionId: checkoutId as string, status: "active" as unknown as string, metadata: meta as unknown as Record<string, unknown> }); } } } } catch (e) { logger.error(`[chargily] handler error ${e instanceof Error ? e.message : String(e)}`); }',
    '  try { await convexClient.mutation(api.billing.upsertWebhookEvent, { provider: "chargily", providerEventId: eventId, type: eventType, payload: payload as unknown as Record<string, unknown>, processed: true }); } catch (e) { logger.error(`[chargily] record failed ${e instanceof Error ? e.message : String(e)}`); }',
...
function baseChargilyTanstack(imp: DbImports): string {
  return [
    ...
    "  const data = (payload.data as unknown as Record<string, unknown>) ?? payload;",
    ...
    "  try { const status = (data.status as string | undefined) ?? (payload.status as string | undefined); const checkoutId = (data.id as string | undefined) ?? (payload.id as string | undefined); if (checkoutId) { if (eventType.includes('paid') || eventType.includes('completed')) { await db.update(checkouts).set({ status: 'completed' as unknown as string }).where(eq(checkouts.providerCheckoutId, checkoutId)); const meta = (data.metadata ?? payload.metadata) as Record<string, unknown> | undefined; const userId = (meta?.userId ?? meta?.chargily_user_id) as string | undefined; if (userId) { await db.insert(subscriptions).values({ userId, provider: 'chargily', providerSubscriptionId: checkoutId, status: 'active', metadata: meta as unknown as Record<string, unknown> }).onConflictDoNothing() } } else if (eventType.includes('failed') || eventType.includes('canceled') || eventType.includes('expired')) { const mapped = eventType.includes('expired') ? 'expired' : 'failed'; await db.update(checkouts).set({ status: mapped as unknown as string }).where(eq(checkouts.providerCheckoutId, checkoutId)); if (!eventType.includes('expired')) { await db.update(subscriptions).set({ status: 'canceled' as unknown as string }).where(eq(subscriptions.providerSubscriptionId, checkoutId)) } } else if (status) { const mapStatus = (s: string): string => (s === 'paid' || s === 'completed') ? 'completed' : (s === 'failed' || s === 'canceled' || s === 'cancelled') ? 'failed' : s === 'expired' ? 'expired' : 'pending'; await db.update(checkouts).set({ status: mapStatus(status) as unknown as string }).where(eq(checkouts.providerCheckoutId, checkoutId)) } } } catch (e) { logger.error(`[chargily] handler error ${e instanceof Error ? e.message : String(e)}`) }",
    "  try { await db.insert(webhook_events).values({ provider: 'chargily', providerEventId: eventId, type: eventType, payload: payload as unknown as Record<string, unknown>, processed: true }).onConflictDoNothing() } catch (e) { logger.error(`[chargily] record fail ${e instanceof Error ? e.message : String(e)}`) }",
...
function baseChargilyTanstackConvex(imp: DbImports): string {
  ...
  return [
    ...
    "  const eventId = (payload.id as string) ?? (data.id as string) ?? `evt_${Date.now().toString(36)}`;",
    "  const eventType = (payload.type as string) ?? `checkout.${data.status ?? 'updated'}`;",
    "  try { const existing = await convexClient.query(api.billing.checkWebhookEvent, { provider: 'chargily', providerEventId: eventId }); if ((existing as unknown as { processed?: boolean })?.processed) return Response.json({ ok: true, alreadyProcessed: true }) } catch (e) { logger.error(`[chargily] idempotency fail ${e instanceof Error ? e.message : String(e)}`) }",
    // Lifecycle parity with the Next.js Convex variant — including subscription upsert.
    "  try { const checkoutId = (data.id ?? payload.id) as string | undefined; if (checkoutId) { if (eventType.includes('paid') || eventType.includes('completed')) { const meta = (data.metadata ?? payload.metadata) as Record<string, unknown> | undefined; await convexClient.mutation(api.billing.upsertCheckout, { provider: 'chargily', providerCheckoutId: checkoutId, status: 'completed' as unknown as string, metadata: meta as unknown as Record<string, unknown> }); const userId = (meta?.userId ?? meta?.chargily_user_id) as string | undefined; if (userId) { await convexClient.mutation(api.billing.upsertSubscription, { userId, provider: 'chargily', providerSubscriptionId: checkoutId, status: 'active' as unknown as string, metadata: meta as unknown as Record<string, unknown> }) } } else if (eventType.includes('failed') || eventType.includes('canceled') || eventType.includes('expired')) { const mapped = eventType.includes('expired') ? 'expired' : 'failed'; await convexClient.mutation(api.billing.upsertCheckout, { provider: 'chargily', providerCheckoutId: checkoutId, status: mapped as unknown as string }) } } } catch (e) { logger.error(`[chargily] handler error ${e instanceof Error ? e.message : String(e)}`) }",
    "  try { await convexClient.mutation(api.billing.upsertWebhookEvent, { provider: 'chargily', providerEventId: eventId, type: eventType, payload: payload as unknown as Record<string, unknown>, processed: true }) } catch (e) { logger.error(`[chargily] record fail ${e instanceof Error ? e.message : String(e)}`) }",
