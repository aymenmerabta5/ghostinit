...
function convexTanstack(imp: DbImports): string {
  ...
  return [
    ...
    "  try { const existing = await convexClient.query(api.billing.checkWebhookEvent, { provider: 'polar', providerEventId: eventId }); if ((existing as any)?.processed) return Response.json({ ok: true, alreadyProcessed: true }) } catch (e) { logger.error(`[polar] idempotency fail`) }",
    "  try { const data = eventPayload as any; if (eventType.includes('subscription')) { const subId = data.id ?? eventId; const userId = data.customerId ?? data.externalCustomerId ?? subId; const statusRaw = data.status ?? 'active'; await convexClient.mutation(api.billing.upsertSubscription, { userId, provider: 'polar', providerSubscriptionId: subId, status: statusRaw as any, metadata: data.metadata }) } } catch (e) { logger.error(`[polar] handler error`) }",
