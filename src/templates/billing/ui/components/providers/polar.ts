...
export function PolarPanel({ billing }: { billing: BillingPageState }): React.JSX.Element {
  const t = useSurfaceTranslations("billing");
  const { subscriptions, invoices, subsLoading, isCheckoutLoading, licenseKey, usageEvents, handleCheckout, handlePortal, copyText } = billing;
