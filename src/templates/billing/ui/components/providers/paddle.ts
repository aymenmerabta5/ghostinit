...
export function PaddlePanel({ billing }: { billing: BillingPageState }): React.JSX.Element {
  const t = useSurfaceTranslations("billing");
  const { subscriptions, invoices, subsLoading, isCheckoutLoading, handleCheckout, handlePortal } = billing;
