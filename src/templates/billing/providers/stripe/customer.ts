...
export async function createStripeCustomer(
  ...
  const requestOptions: Stripe.RequestOptions | undefined = input.idempotencyKey
    ? {
        idempotencyKey: `ghostinit_customer_${createHash("sha256").update(input.idempotencyKey).digest("hex")}`,
      }
    : undefined;
