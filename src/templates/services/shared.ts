...
export function resultModuleForMode(mode: ProjectMode): string {
  return mode === "monorepo" ? "@repo/kernel" : "@/server/kernel/result";
}
export function resultImportForMode(mode: ProjectMode): string {
  const module = resultModuleForMode(mode);
  return `import type { Result } from "${module}";\nimport { err, ok } from "${module}";`;
}
