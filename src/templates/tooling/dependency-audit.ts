// @allow-long 660: one generated, fail-closed dependency verifier keeps its trust roots and parser auditable
...
export function dependencyAuditScriptContent(hasImageSizePatch: boolean): string {
  return `// @allow-long 560: one fail-closed verifier keeps lock parsing, graph traversal, and audit policy together
...
export function dependencyAuditFiles(hasImageSizePatch: boolean): TemplateFile[] {
  return [
    file("scripts/audit-dependencies.ts", dependencyAuditScriptContent(hasImageSizePatch)),
    file(".gitattributes", generatedGitattributesContent(hasImageSizePatch)),
