...
function canonicalize(value: unknown, ancestors: Set<object>): string {
  ...
  if (Array.isArray(value)) {
    ...
    ancestors.add(value);
    const result = `[${value.map((entry) => canonicalize(entry, ancestors)).join(",")}]`;
...
/** A runtime-neutral SHA-256 implementation used only for deterministic plan identities. */
export function sha256(value: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h + sum1 + choice + (SHA_256_CONSTANTS[index] ?? 0) + (schedule[index] ?? 0)) >>> 0;
...
export function deepFreeze<T>(value: T): T {
  ...
  for (const key of Reflect.ownKeys(value)) deepFreeze(record[key]);
